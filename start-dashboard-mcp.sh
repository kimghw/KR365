...
# Cloudflare는 별도 cloudflare.sh 스크립트로 관리
...
# Cloudflare 관련 함수들은 cloudflare.sh 스크립트 사용
# cloudflare.sh 실행: ./cloudflare.sh
...
case "${1:-start-dashboard}" in
    ...
    *)
        ...
        echo "  start-all             - Start FastAPI and Dashboard"
        ...
        echo ""
        echo "💡 Cloudflare Tunnel 관리는 ./cloudflare.sh 사용"
        exit 1
        ;;
esac
