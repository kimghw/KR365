...
case "${1:-start-dashboard}" in
    ...
    *)
        ...
        echo ""
        echo "  # Start OneNote server with Cloudflare tunnel on port 8002"
        echo "  SERVER_TYPE=onenote ONENOTE_SERVER_PORT=8002 ./start-dashboard-mail-query.sh start-all"
        echo ""
        echo "  # Switch server type and restart Cloudflare tunnel"
        echo "  SERVER_TYPE=mail_query ./start-dashboard-mail-query.sh restart-cloudflare"
        echo ""
        echo "Default action: start-dashboard (dashboard only)"
        echo ""
        echo "💡 Cloudflare tunnel will automatically update to proxy the selected MCP server port"
        exit 1
        ;;
esac
