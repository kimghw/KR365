...
class FastAPIMailAttachmentServer:
    ...
    def _create_app(self):
        ...
        from modules.mail_query_MCP.middleware.request_logger import RequestLoggerMiddleware
        import os
...
```
        ...
        async def stream_alias(request: Request, user_data: dict = Depends(required_auth)):
            """Stream endpoint alias for compatibility with required DCR authentication"""
            logger.info(f"🔐 Authenticated request from user: {user_data.get('user_id')}")
            request.state.user_id = user_data.get('user_id')
            return await self._handle_mcp_request(request)
        ...
        async def steam_alias(request: Request, user_data: dict = Depends(required_auth)):
            """Steam endpoint alias for compatibility with required DCR authentication"""
            logger.info(f"🔐 Authenticated request from user: {user_data.get('user_id')}")
            request.state.user_id = user_data.get('user_id')
            return await self._handle_mcp_request(request)
        ...
        async def auth_login_handler(user_id: str = None):
            ...
            from modules.mail_query_MCP.implementations.auth_session_store import AuthSessionStore
