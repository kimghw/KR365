...
class DCRAuthenticator:
    ...
    def dcr_service(self):
        ...
        if self._dcr_service is None:
            ...
            self._dcr_service = DCRService(server_name=self.server_name)
            logger.info(f"✅ DCR Service initialized with server_name: {self.server_name}, database: auth_{self.server_name}.db")
    ...
    async def verify_token(
        ...
        excluded_paths = [
            ...
            "/redoc",
            "/openapi.json",
            "/auth/callback",            # 레거시 OAuth 콜백
            "/auth/login",               # 레거시 OAuth 로그인
            "/info"
        ]
        # 주의: /oauth/token은 의도적으로 제외하지 않음 (클라이언트 인증 필요)
        ...
        if not credentials:
            ...
            # log_dcr_middleware는 선택적 파라미터를 가짐
